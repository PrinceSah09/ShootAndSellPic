...
const ProductList: React.FC = () => {
  ...
  const filteredItem = productList.map((item) => {
    ...
    return cartItem ? { ...item, inCart: true } : { ...item, inCart: false };
  });

  // State to manage products and search query
  const [products, setProducts] = useState<Product[]>(filteredItem);
  ...
  const handleBuyNow = (productId: number) => {
    ...
    // Dispatch the buyNowItem action with the selected product
    if (product) {
      dispatch(
        buyNowItem({
          id: product.id,
